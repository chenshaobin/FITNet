...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        ...
        # print('Wq shape:{}, Wkvshape:{}'.format(self.to_q, self.to_kv))
    ...
    def forward(self, x, context = None, kv_include_self = False):
        # print('q input shape:', x.shape)  # (batch_size, num_patches + 1(or 1 when use crossAttention), sm_dim(lg_dim))
        ...
        # print('attention output shape:', self.to_out(out).shape)
...
class ProjectInOut(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        x = self.project_in(x)
        # print('ProjectInout first output shape:', x.shape)
        x = self.fn(x, *args, **kwargs)
        # print('ProjectInout attention output shape:', x.shape)
        x = self.project_out(x)
        # print('ProjectInout final output shape:', x.shape)
        return x

# interactive attention transformer

class IATransformer(nn.Module):
    ...
    def forward(self, sm_tokens, lg_tokens):
        ...
        # print('sm_cls shape:{}, lg_cls shape:{}'.format(sm_cls.shape, lg_cls.shape))
        ...
        *,
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                IATransformer(sm_dim = sm_dim, lg_dim = lg_dim, depth = cross_attn_depth, heads = cross_attn_heads, dim_head = cross_attn_dim_head, dropout = dropout)
            ]))
...
class ImageEmbedder(nn.Module):
    ...
    def forward(self, img):
        ...
        # print('final patch_embedding shape:', x.shape)
...
class IAUnit(nn.Module):
    ...
    def forward(self, feature1, feature2):
        ...
        # print('sm_tokens shape:{}, lg_tokens shape:{}'.format(sm_tokens.shape, lg_tokens.shape))
        ...
        print('crossVit output shape:{}, output:{}'.format((sm_logits + lg_logits).shape, sm_logits + lg_logits))
        return sm_logits + lg_logits
        """
        fusion_cls = torch.cat((sm_cls, lg_cls), dim=-1)
        # print('fusion_cls shape:', fusion_cls.shape)
        return fusion_cls
...
if __name__ == '__main__':
    ...
    # print(m(pre2))
    finalPre = torch.cat((pre1, pre2), dim=-1)
    # print('finalPre shape:{}'.format(finalPre.shape))
